...
全身ポーズは、全キャラクターにそろっている次の5つの**場面の意味**を優先します。ファイル名はキャラクターにより異なるため、必ず`catalog.json`で確認します。
...
実際に使う前に、下のKai用のIDとポーズが`catalog.json`にあることを確認します。他のキャラクターに替えるときは、同じ場面に対応する実在ファイル名をカタログから選び直します。
