...
const grid=document.querySelector('#characterGrid'),library=document.querySelector('#libraryCard'),assetGrid=document.querySelector('#assetGrid'),groupGrid=document.querySelector('#groupGrid'),usage=document.querySelector('#usageCard'),usageImage=document.querySelector('#usageImage'),usageTitle=document.querySelector('#usageTitle'),usageCode=document.querySelector('#usageCode'),copyButton=document.querySelector('#copyButton'),copyStatus=document.querySelector('#copyStatus'),webDownload=document.querySelector('#webDownload'),originalDownload=document.querySelector('#originalDownload'),title=document.querySelector('#libraryTitle'),count=document.querySelector('#libraryCount');
...
function renderCards(){grid.innerHTML=characters.map((c,i)=>`<article class="character-card ${selected?.id===c.id?'selected':''}" data-id="${c.id}"><button class="character-button" type="button"><img class="character-image" src="${webAssetPath(c.id,'fullbody','waving')}" alt="${c.label}"><div class="character-info"><div class="character-index">CHARACTER ${String(i+1).padStart(2,'0')}</div><h3>${c.label}</h3><p>${c.fullbody.length}ポーズ / ${c.expressions.length}表情</p></div></button></article>`).join('');}
function renderAssets(){if(!selected)return;const names=selected[tab];assetGrid.innerHTML=names.map(name=>{const webPath=webAssetPath(selected.id,tab,name);const originalPath=originalAssetPath(selected.id,tab,name);return`<button class="asset-item" data-path="${webPath}" data-original="${originalPath}" data-label="${selected.label}・${name}"><img src="${webPath}" alt="${selected.label} ${name}"><span>${esc(name)}</span></button>`}).join('');count.textContent=`${names.length} files`;}
...
groups.forEach(([name,label])=>{const ext=name==='group-jumping'?'png':'webp';const originalExt=name==='group-jumping'?'jpg':'png';const webPath=`assets/web/groups/${name}.${ext}`;const originalPath=`assets/groups/${name}.${originalExt}`;groupGrid.insertAdjacentHTML('beforeend',`<button class="group-item" data-path="${webPath}" data-original="${originalPath}" data-label="${label}"><img src="${webPath}" alt="${label}"><span>${label}<small>${name}.${ext} / 原本をダウンロード可</small></span></button>`);});
